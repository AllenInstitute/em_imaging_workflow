...
class SolveRoughAlignmentStrategy(ExecutionStrategy):
    ...
    def clip_z_mapping_to_min_max(cls, z_mapping, min_z, max_z):
        ...
        for old_z in range(min_z, max_z+1):
            str_z = str(old_z)

            if str_z in z_mapping:
                clipped_mapping[str_z] = z_mapping[str_z]
    ...
    def get_input(self, chnk, storage_directory, task):
        ...
        zs = [int(z) for z in clipped_z_mapping.keys()]
        z_start = min(zs)
        z_end = max(zs)
