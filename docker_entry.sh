...
python -m celery flower --backend=rpc:// --broker=amqp://blue_sky_user:blue_sky_user@message_queue:5672 --name=flower@at_em_image_processing &
python -m celery -A execution_runner worker --loglevel=debug --concurrency=2 --name=at_em_worker@at_em_image_processing &
